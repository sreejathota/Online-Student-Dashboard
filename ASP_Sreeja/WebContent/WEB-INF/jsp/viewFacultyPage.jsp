...
<h1 class="w3-text-teal" style="margin-top:10px">View Faculty</h1>
<span>${errormessage}</span>
      <table class="myTable" style="margin-top:35px;margin-left:100px"> 

	<tr ><th>Faculty ID</th><th>Faculty name</th><th>Mail ID</th><th>Assign Course</th></tr>
   <c:forEach var="faculty" items="${displayFaculty}">   
   <tr >  
   <td>${faculty.facultyId}</td>  
   <td>${faculty.facultyName}</td>  
   <td>${faculty.mailId}</td>  
   <td><a href="/ASP_Sreeja/adminController/assignCourse/${faculty.facultyId}">Assign Course</a></td>
   </tr>  </c:forEach> 
 ...
 <!-- Pagination -->
  <div class="w3-center w3-padding-64">
   ...
   <c:url value="/adminController/viewfaculty" var="prev">
    ...
    <c:forEach begin="1" end="${maxPages}" step="1" varStatus="i">
        <%-- <c:choose>
            ...
            <c:otherwise> --%>
                <c:url value="/adminController/viewfaculty" var="url">
    ...
    <c:url value="/adminController/viewfaculty" var="next">
        <c:param name="page" value="${page + 1}"/>
    </c:url>
    <c:if test="${page + 1 <= maxPages}">
       <li><a class="w3-black" href='<c:out value="${next}" />' >Next</a></li>
    </c:if>
    </ul>
    </div>
